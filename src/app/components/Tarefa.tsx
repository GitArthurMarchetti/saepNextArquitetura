...
console.log(tarefas)
    ...
    return (
        <div className="relative min-h-screen bg-gray-100">
            <div className="p-6">
                <h1 className="text-2xl font-bold text-center text-black mb-6">Kanban</h1>
                <div className="grid grid-cols-3 gap-6">
                    {/* Coluna A Fazer */}
                    <div className="bg-white p-4 rounded-lg shadow-md">
                        <h2 className="text-lg font-bold mb-4 text-black">A Fazer</h2>
                        <ScrollArea>
                            <div className="space-y-4">
                                {tarefasAFazer.map((tarefa) => (
                                    <div
                                        key={tarefa.id}
                                        className="relative bg-gray-50  border-4 border-[#0056b3] rounded-lg p-4 shadow-sm"
                                    >
                                        <button
                    ...
                    {/* Coluna Processando */}
                    <div className="bg-white p-4 rounded-lg shadow-md">
                        <h2 className="text-lg font-bold mb-4 text-black">Processando</h2>
                        <div className="space-y-4">
                            {tarefasProcessando.map((tarefa) => (
                                <div
                                    key={tarefa.id}
                                    className="relative  border-4 border-[#0056b3] bg-gray-50 rounded-lg p-4 shadow-sm"
                                >
                                    <button
                                        className="absolute top-2 right-2 text-red-500 hover:text-red-600"
                                        title="Excluir"
                    ...
                    {/* Coluna Concluído */}
                    <div className="bg-white p-4 rounded-lg shadow-md">
                        <h2 className="text-lg font-bold mb-4 text-black">Concluído</h2>
                        <div className="space-y-4">
            ...
            <div className="fixed bottom-6 right-6 flex flex-col gap-4">
                ...
                <div className="bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg hover:bg-green-600 transition">
                    <SheetComponent tarefas={tarefas} tarefa={tarefaParaEditar || tarefa} users={users} />
                </div>
            </div>
        </div>
    );
